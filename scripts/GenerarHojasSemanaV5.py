...
import os
from mostrarNicks import getNick, getApNick  # Importamos las funciones para obtener los nicks y apellidos registrados
...
def crear_hoja(ws, intervalos, horarios_dia, tipo_personal, dia):
    # Encabezado con apellidos y nicks en las primeras filas
    ...
    for col, (nombres, apellidos) in enumerate(horarios_dia.keys(), start=2):
        ...
        # Obtener el nick y apellido registrado usando las funciones
        if getNick(nombre_completo) != "ERROR NR":
            nick = getNick(nombre_completo)
        else:
            nick = nombres.split()[0]
        if getApNick(nombre_completo) != "ERROR NR":
            apellido_registrado = getApNick(nombre_completo)
        else:
            apellido_registrado = " ".join(apellidos.split()[:2] if len(apellidos.split()[0]) == 2 else apellidos.split()[:1])
        if "APO" in apellido_registrado:
            apellido_registrado = "APO"
        # Colocar el apellido registrado en la primera fila y el nick en la segunda fila
        ws.cell(row=1, column=col).value = apellido_registrado
        ws.cell(row=2, column=col).value = nick

    # Segunda fila con los intervalos
    ws["A2"] = "Intervalos"
    ...
    # Contar el personal en turno por intervalo (basado en las celdas coloreadas)
    for i, intervalo in enumerate(intervalos, start=3):
        count = sum(1 for col in range(2, len(horarios_dia) + 2) 
                    if ws.cell(row=i, column=col).fill.start_color.index != '00000000')  # Verifica si está coloreada
