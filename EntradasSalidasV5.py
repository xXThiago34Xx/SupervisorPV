    ...
    if ver_por == "1":
        ...
        for registros in horarios_dia.values():
            for registro in registros:
                if len(registro) == 4:
                    apellidos, nombres, entrada, salida = registro
                    salidas.append((entrada, salida, apellidos, nombres))
        #Limpiar Duplicados
        salidas = list(set(salidas))
...
def main():
    ...
    tipos_personal = ["Self Checkout", "RS", "Cajer@", "Ecommerce", "Supervisor(@)"]  # Añadida la nueva categoría
    ...
    while True:
        ...
        salir = int(input("\n¿Desea salir? (1 para sí, 0 para no): ").strip())
        if salir == 1:
            break
