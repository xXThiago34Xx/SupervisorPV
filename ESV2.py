...
def llenar_entradas(ws, Entradas):
    ...
    for i in range(1, 73):  # Recorrer las horas de entrada (1 a 72)
        ...
        for j, entrada in enumerate(Entradas[i]):
            # Solo colocar en las primeras 6 columnas (B a G) si hay 2 elementos (nombre y tipo)
            if j < 6 and len(entrada) == 2:
                nombre, tipo_personal = entrada

                # Definir la celda correspondiente en la hoja
                # Desde la columna B (que es la columna 2)
                celda_columna = get_column_letter(j + 2)
                ...
                if tipo_personal in roles_colores:
                    fill = PatternFill(
                        start_color=roles_colores[tipo_personal], end_color=roles_colores[tipo_personal], fill_type='solid')
                    ws[celda].fill = fill
...
def llenar_salidas(ws, Salidas):
    ...
    for i in range(1, 73):  # Recorrer las horas de entrada (1 a 72)
        ...
        for j, salida in enumerate(Salidas[i]):
            # Solo colocar en las primeras 6 columnas (B a G) si hay 2 elementos (nombre y tipo)
            if j < 7 and len(salida) == 2:
                nombre, tipo_personal = salida

                # Definir la celda correspondiente en la hoja
                # Desde la columna B (que es la columna 2)
                celda_columna = get_column_letter(j + 2 + 7)
                celda_fila = i + 2  # Fila empieza en 3 porque fila 1 y 2 son los encabezados
                celda = f"{celda_columna}{celda_fila}"
...
def rellenar(ws, celda, texto='', color_rgb=''):
    ...
    if color_rgb:
        fill = PatternFill(start_color=color_rgb,
                           end_color=color_rgb, fill_type='solid')
        ws[celda].fill = fill
...
def crear_tabla_excel(Entradas, Salidas, dia):
    ...
    combinar_y_centrar_celdas(
        ws, "A2", "G2", "Horas de Entrada del Personal del Area de Cajas")
    combinar_y_centrar_celdas(
        ws, "H2", "O2", "Horas de Salida del Personal del Area de Cajas")
    ...
    # Ajusta el rango según sea necesario
    for row in ws.iter_rows(min_row=1, max_row=74, min_col=1, max_col=15):
        for cell in row:
            cell.border = border
...
def obtener_horarios_por_dia(horarios, dia, tipo_personal):
    dia_index = ["lunes", "martes", "miércoles", "jueves",
                 "viernes", "sábado", "domingo"].index(dia.lower())
    ...
    for registro in horarios:
        ...
        if cargo.lower() == tipo_personal.lower():
            ...
            if entrada_formateada == "DESCANSO" and salida_formateada == "DESCANSO":
                ...
                horarios_dia[entrada_formateada].append(
                    (apellidos, nombres, entrada_formateada, salida_formateada))
                horarios_dia[salida_formateada].append(
                    (apellidos, nombres, entrada_formateada, salida_formateada))
...
def ESaMEMORIA(horarios_dia, tipoPersonal):
    ...
    for hora, registros in horarios_dia.items():
        ...
        for apellidos, nombres, entrada, salida in registros:
            if hora == entrada:
                horarios_entradas[hora].append(
                    (apellidos, nombres, entrada, salida))
            elif hora == salida:
                horarios_salidas[hora].append(
                    (apellidos, nombres, entrada, salida))
    ...
    for hora in sorted(set(horarios_entradas.keys()).union(horarios_salidas.keys())):
        ...
        if hora in horarios_entradas:
            ...
            for apellidos, nombres, entrada, salida in sorted(horarios_entradas[hora], key=lambda x: x[1]):
                ...
                Empilar(Entradas, getNHora(entrada),
                        nickname_completo, tipoPersonal)
        if hora in horarios_salidas:
            ...
            for apellidos, nombres, entrada, salida in sorted(horarios_salidas[hora], key=lambda x: x[1]):
                ...
                ap = getApNick(nombre_completo)
                nickname_completo = f"{nick}"  # f"{nick} {ap}"
                Empilar(Salidas, getNHora(salida),
                        nickname_completo, tipoPersonal)
...
tipos_personal = ["Cajer@", "RS",
                  "Self Checkout", "Ecommerce", "Supervisor(@)"]
print("\nSeleccionar Día:")
