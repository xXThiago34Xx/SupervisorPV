# añadir autofit a ecommerce y self
# ando probando con rs, rs colocado en horizontal
# hojas centradas verticalmente y horizontalmente
...
def adjust_column_width(sheet, sheet_name):
    # Ajusta solo la primera columna si no contiene "Ecommerce" o "Self"
    if "Ecommerce" in sheet_name or "Self" in sheet_name or "RS" in sheet_name or "Supervisor(@)" in sheet_name:
        sheet.Columns.AutoFit()  # Ajusta el ancho de todas las columnas
    else:
        sheet.Columns(1).AutoFit()  # Ajusta solo la primera columna
...
def process_excel_to_pdf(excel_path, output_dir):
    ...
    for sheet in workbook.Sheets:
        if sheet.UsedRange.Count > 1:  # Omitir hojas sin contenido
            ...
            adjust_column_width(sheet, sheet_name)  # Ajusta el ancho de las columnas según la hoja
    ...
    # Agrupar las hojas en bloques de 5
    for i in range(0, len(all_pdfs), 5):
        group_pdfs = all_pdfs[i:i+5]
        day_pdf_path = os.path.join(days_folder, f"Grupo_{(i // 5) + 1}.pdf")
        merger = PdfMerger()
