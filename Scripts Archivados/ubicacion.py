...
def asignar_cajeros(ubicaciones):
    ...
    for ubicacion in sorted(ubicaciones, key=lambda x: x['horaEntrada']):
        # Asignar a la Caja 1 sin dejar huecos
        if len(cajas[1]) == 0:
            cajas[1].append(ubicacion)
        else:
            ultimo_cajero = cajas[1][-1]
            if ultimo_cajero["horaSalida"] <= ubicacion["horaEntrada"]:
                cajas[1].append(ubicacion)

        # Asignar a las demás cajas
        for caja_num in range(2, 16):
            if len(cajas[caja_num]) == 0 or (cajas[caja_num][-1]["horaSalida"] <= ubicacion["horaEntrada"]):
    ...
    for caja_num in rapidas.keys():
        if len(rapidas[caja_num]) == 0:
            for ubicacion in ubicaciones:
                if ubicacion["horaEntrada"] >= datetime.strptime("09:00", "%H:%M"):
...
def imprimir_resultados(cajas, rapidas):
    for caja_num, cajeros in cajas.items():
        ...
        for i, cajero in enumerate(cajeros):
            sobra = (cajero['horaSalida'] - cajero['horaEntrada']).seconds // 60
            # Si no es el último cajero, calculamos el tiempo que "sobra"
            if i < len(cajeros) - 1:
